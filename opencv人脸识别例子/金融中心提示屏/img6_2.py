...
def playMusic():
    # while True:
    global child
    child = subprocess.Popen(["python", "backMusic.py", "music.mp3"])
    return child.pid
pid = playMusic()
print(pid)
...
cv.resizeWindow('imgWindow', 1920, 1080)
...
try:
    while True:
        ...
        font = ImageFont.truetype('simhei.ttf', 200) #文字字体和大小
        fillColor = (189, 189, 189) #文字颜色
        position = (2350, 100) #文字位置
        strTxt = "扫码后请至前台咨询客服打印并下载您的照片"
        ...
        # 垂直叠加
        imgs = np.vstack((imgs_1, imgs_2, txtImg)) #目前H = 4820, W = 8640
